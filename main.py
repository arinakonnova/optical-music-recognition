...
def resize(img, scale): 
    '''
    Resizes an image using the scale passed in parameter.
    @img - opencv image file

    @scale - desired height of the result

    @returns the resized open cv image
    '''
    h, w = img.shape[:2]
    coeff = scale / h
...
def findSymbol(img,template,threshold):
    '''
    Uses cv2.matchTemplate on img with the given template, then applies a threshold to elminate false-positives

    @returns array of coordinates with the following form: [x,y,w,h]
    '''
...
def remove_inner_boxes(a1, a2):
    '''
    Removes from a2 the rectangles that overlap with a1

    @returns a2 with the overlaps removed
    '''
    keep = []
...
def delete_out_of_staff(rectangles, staves):
    '''
    Deletes every rectangle from rectangles whose y coordinates are not within one of the staves

    @returns updated rectangles
    '''
    filtered = []
    for p in rectangles:
        found = False
...
def delete_left(rectangles,left):
    '''
    Deletes every rectangle from rectangles whose x is smaller than left
...
def sort_in_staff(staff_notes, staves, found, duration, type):
    '''
    Goes through every rectangle found, creates a note object with the given attributes then sorts it in the right staff

    @returns updated staff_notes
    '''
    for p in found:
        x,y,w,h = p
...
for staff in staves: 
   ys = sorted(staff)
   #padding above and under the staff
   top = ys[0] - 10
   bottom = ys[-1] + 10
...
# ===================================================================================================================
# STEP 2: Finding the symbols in the sheet
# ===================================================================================================================

# importing & resizing all the needed templates
...
#We call find symbol on all the template, each with their own thresholds
...
#deleting everything on the left that might overlap with clefs and key signatures
...
#draw a rectangle on the staffless image to get a summary of all the symbols
...
# ===================================================================================================================
# STEP 3: Converting the symbols, organize them into the staves 
# ===================================================================================================================
...
# ===================================================================================================================
# STEP 4: Find the right pitch for each note
# ===================================================================================================================


# Values of pitch that a note can have, depending on the type of staff
...
# ===================================================================================================================
# STEP 5 : Creating a MIDI file, transfering the notes on it.
# ===================================================================================================================


# fusing everything into 2 arrays of notes that will symbolize the two tracks
treble_notes = []
bass_notes = []
...
MyMIDI = MIDIFile(numTracks=2)  # One track, defaults to format 1 (tempo track is created automatically)
...
for i in range (len(bass_notes)):
    ...
    if note.duration!=0:
        ...
        #Homophony, if two notes have the same coordinates, we play them at the same time
